...
=========================
All configuration is loaded from config.json at startup.

config.json keys (harvester settings):
    CUSTOM         bool   - Use custom browser executable
    PATH           str    - Path to browser executable
    N              int    - Number of windows (TABS=False) or tabs (TABS=True)
    TABS           bool   - False = N separate windows | True = N tabs in one window
    EXTENSIONS     bool   - Load browser extensions
    EXTENSIONS_DIR str    - Path to browser Extensions folder
    CUS_PROFILE    bool   - Use custom browser profile
    PROFILE_PATH   str    - Path to browser user data directory
    COOKIES        bool   - Inject auth cookies manually
    COOKIE_V1      str    - Value for arena-auth-prod-v1.0
    COOKIE_V2      str    - Value for arena-auth-prod-v1.1
    AUTO_LOGIN     bool   - Auto sign-in via arena.ai API
    SERVER_PORT    int    - Local server port (default 5000)
    TOKENS_FILE    str    - Output file (default tokens.json)
...
CUSTOM         = _cfg.get("CUSTOM",        True)
PATH           = _cfg.get("PATH",          "")
N              = int(_cfg.get("N",         1))
TABS           = _cfg.get("TABS",          False)
EXTENSIONS     = _cfg.get("EXTENSIONS",    False)
EXTENSIONS_DIR = _cfg.get("EXTENSIONS_DIR","")
CUS_PROFILE    = _cfg.get("CUS_PROFILE",   False)
PROFILE_PATH   = _cfg.get("PROFILE_PATH",  "")
COOKIES        = _cfg.get("COOKIES",       False)
COOKIE_V1      = _cfg.get("COOKIE_V1",     "")
COOKIE_V2      = _cfg.get("COOKIE_V2",     "")
AUTO_LOGIN     = _cfg.get("AUTO_LOGIN",    False)
SERVER_PORT    = int(_cfg.get("SERVER_PORT", 5000))
TOKENS_FILE    = _cfg.get("TOKENS_FILE",   "tokens.json")
...
if CUS_PROFILE:
    if not PROFILE_PATH or not str(PROFILE_PATH).strip():
        raise RuntimeError(
            "CUS_PROFILE=True but PROFILE_PATH is empty in config.json.\n"
            "Set PROFILE_PATH to your browser's user data directory."
        )
    _profile_path_obj = Path(str(PROFILE_PATH).strip())
...
if COOKIES:
    if not COOKIE_V1 or not str(COOKIE_V1).strip():
        raise RuntimeError(
            "COOKIES=True but COOKIE_V1 is empty in config.json.\n"
            "Set COOKIE_V1 to the value for the arena-auth-prod-v1.0 cookie."
        )
    if not COOKIE_V2 or not str(COOKIE_V2).strip():
        raise RuntimeError(
            "COOKIES=True but COOKIE_V2 is empty in config.json.\n"
            "Set COOKIE_V2 to the value for the arena-auth-prod-v1.1 cookie."
        )

if AUTO_LOGIN and COOKIES:
    raise RuntimeError(
        "AUTO_LOGIN=True and COOKIES=True cannot be used together in config.json.\n"
...
(function() {
  ...
  (async function() {
    try {
      ...
      fetch('http://localhost:""" + str(SERVER_PORT) + """/api', {
        method: 'POST',
...
V2_SCRIPT = r"""
(() => {
    const SERVER_URL   = "http://localhost:""" + str(SERVER_PORT) + r"""/api";
...
async (windowId) => {
    try {
        await fetch('http://localhost:""" + str(SERVER_PORT) + """/' + windowId + '/ready', { method: 'POST' });
...
def _get_extension_args() -> list[str]:
    ...
    if not EXTENSIONS_DIR or not str(EXTENSIONS_DIR).strip():
        raise RuntimeError(
            "EXTENSIONS=true but EXTENSIONS_DIR is empty in config.json.\n"
            "Set EXTENSIONS_DIR to your browser's Extensions folder path."
        )

    base = Path(str(EXTENSIONS_DIR).strip())
    if not base.exists():
...
def _resolve_profile_dir(slot: str) -> Path:
    if CUS_PROFILE:
        return Path(str(PROFILE_PATH).strip())
    d = PROFILES_DIR / slot
...
async def main():
    ...
    print(f"  Custom browser : {CUSTOM}{(' → ' + str(PATH)) if CUSTOM and PATH else ''}")
